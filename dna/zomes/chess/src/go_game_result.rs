...
#[hdk_entry(id = "go_game_result")]
#[derive(Clone)]
pub struct GoGameResult {
...
pub fn get_my_game_results() -> ExternResult<Vec<(EntryHashB64, GoGameResult)>> {
    ...
    let results = links
        ...
        .collect::<ExternResult<Vec<(EntryHashB64, GoGameResult)>>>()?;

    Ok(results)
}

fn get_game_result(game_result_hash: EntryHash) -> ExternResult<GoGameResult> {
    let element = get(game_result_hash, GetOptions::default())?
        .ok_or(WasmError::Guest("Could not get game result".into()))?;

    let game_result: GoGameResult = element
        .entry()
        .to_app_option()?
        .ok_or(WasmError::Guest("Could not get game result".into()))?;
